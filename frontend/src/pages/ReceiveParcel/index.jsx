...
import { checkReceivedSerial, receiveParcel } from './service';
...
const EntryForm = () => {
  ...
  const handleChange = (pTags, changed) => {
    ...
    if (tags.length < newTags.length) {
      checkReceivedSerial({}, parseInt(changed[0]))
        .then((res) => {
          console.log(res);
          if (res.success) {
            setTags(newTags);
        ...
        .catch((err) => message.error('Invalid ID'));
  ...
  const handleSubmit = async (data) => {
    try {
      ...
      const resp = await receiveParcel(subData);
